...
salt_status_t salt_status = {0};

salt_control_t salt_control = {0};
...
void salt_operate(salt_key_id_t key_id, void *mqtt_client, bool force_resend, ContainerTypeEnum container_type)
{
    ...
    for (uint16_t cid = CID_SALT_ELECTROLISE; (err != ESP_ERR_NOT_FOUND) && cid <= CID_SALT_PRODUCTION_PERCENT; cid++)
    {
