...
esp_err_t modem_board_on(void)
{
    ...
    io_conf.pin_bit_mask = BIT64(GPIO_NUM_7);
...
esp_err_t modem_board_data_request(esp_modem_dce_t *dce)
{
    esp_modem_dce_set_command_mode(dce, NULL, NULL);
    ESP_LOGW(TAG, "Mode changed");
    esp_modem_dce_get_imei_number(dce, NULL, NULL);
    vTaskDelay(pdMS_TO_TICKS(500));
    esp_modem_dce_get_imsi_number(dce, NULL, NULL);
    vTaskDelay(pdMS_TO_TICKS(500));
    esp_modem_dce_resume_data_mode(dce, NULL, NULL);
...
static esp_err_t my_recov(esp_modem_recov_resend_t *retry_cmd, esp_err_t err, int timeouts, int errors)
{
    ...
    if (err == ESP_ERR_TIMEOUT)
    {
        if (timeouts < 2)
        {
            ...
            esp_modem_dce_reset(dce, NULL, NULL);
        ...
        else
        {
            esp_modem_dce_set_command_mode(dce, NULL, NULL);
            ESP_LOGW(TAG, "Mode changed");
            ESP_LOGI(TAG, "Restart AT command");
            esp_modem_dce_reset(dce, NULL, NULL);
...
esp_netif_t *modem_board_init(modem_config_t *config)
{
    ...
    //Check if 4G dongle is connected and return result 
    if (dte == NULL)
    {
        ESP_LOGI(TAG,"GWID_IS4G:NO");
        return NULL;
    }
    ESP_LOGI(TAG,"GWID_IS4G:YES");
    assert(dte != NULL);
    ...
    led_power_up_test();
    set_led_behaviour(Wifi, FastBlinking);
    ESP_ERROR_CHECK(esp_modem_default_attach(dte, dce, ppp_netif));
    ESP_ERROR_CHECK(esp_modem_default_start(dte));

    esp_err_t ret = ESP_OK;
    esp_modem_print_info(dte);
    vTaskDelay(pdMS_TO_TICKS(1000));
    
    if(ret == ESP_OK)
    {
        ESP_LOGI(TAG, "Modem get info is ok");
    }
    else
    {
        ESP_LOGI(TAG, "Modem get info error");
    }

    ret = esp_modem_start_ppp(dte);
    ESP_LOGI(TAG, "AFTER esp_modem_start_ppp");
    int dial_retry_times = CONFIG_MODEM_DIAL_RERTY_TIMES;
    EventBits_t event_bits = xEventGroupWaitBits(connection_events, CONNECT_BIT, pdFALSE, pdTRUE, pdMS_TO_TICKS(60000));
    ESP_LOGI(TAG, "event_bits = %d", event_bits);
    bool is_ppp_connected = event_bits & CONNECT_BIT;
    while (!is_ppp_connected && --dial_retry_times > 0)
    {
        ret = esp_modem_stop_ppp(dte);
        vTaskDelay(pdMS_TO_TICKS(1000));
        ret = esp_modem_start_ppp(dte);
        ESP_LOGI(TAG, "re-start ppp, retry=%d", dial_retry_times);
        event_bits = xEventGroupWaitBits(connection_events, CONNECT_BIT, pdFALSE, pdTRUE, pdMS_TO_TICKS(60000));
        ESP_LOGI(TAG, "event_bits = %d", event_bits);
        is_ppp_connected = event_bits & CONNECT_BIT;
    };
    ...
    if (ret == ESP_OK)
    {
        set_led_behaviour(Wifi, StopFastBlinking);
    }
    else
    {
        if (led_system_handle)
