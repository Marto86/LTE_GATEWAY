...
esp_err_t esp_modem_dce_reset(esp_modem_dce_t *dce, void *param, void *result)
{
    ...
    err = esp_modem_dce_generic_command(dce, "AT+RESET\r", MODEM_COMMAND_TIMEOUT_RESET, esp_modem_dce_handle_reset, NULL);
    if (err != ESP_OK)
    {
    return esp_modem_dce_generic_command(dce,  "AT^RESET\r", MODEM_COMMAND_TIMEOUT_RESET, esp_modem_dce_handle_reset, NULL);
    }
    return err;
}
...
esp_err_t esp_modem_dce_set_flow_ctrl(esp_modem_dce_t *dce, void *param, void *result)
{
    ...
    int len = asprintf(&command, "AT&K=%d,%d\r", dte->flow_ctrl, flow_ctrl);
    if (len <= 0) {
