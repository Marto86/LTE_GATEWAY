...
esp_err_t esp_modem_dce_handle_response_cgsn(esp_modem_dce_t *dce, const char *line)
{
    ...
    ESP_LOGI(TAG, "GWID_IMEI:%s", line);
    return err;
}

esp_err_t esp_modem_dce_handle_response_cimi(esp_modem_dce_t *dce, const char *line)
{
    esp_err_t err = ESP_FAIL;
    if (strstr(line, MODEM_RESULT_CODE_SUCCESS)) {
        err = esp_modem_process_command_done(dce, ESP_MODEM_STATE_SUCCESS);
    ...
    ESP_LOGI(TAG, "GWID_SIMSN:%s", line);
    return err;
}

esp_err_t esp_modem_print_info(esp_modem_dte_t *dte)
{
    esp_modem_dce_t *dce = dte->dce;
    ESP_MODEM_ERR_CHECK(dce, "esp_modem_get_info DTE has not yet bind with DCE", err);
    esp_modem_dce_generic_command(dce, "AT+CGSN\r", 1500, esp_modem_dce_handle_response_cgsn, NULL);
    esp_modem_dce_generic_command(dce, "AT+CIMI\r", 1500, esp_modem_dce_handle_response_cimi, NULL);
